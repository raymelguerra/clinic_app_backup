...
export class ContractorComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    merge(this.contractorForm.valueChanges, ...controlBlurs)
  ...
  getContractors() {
    ...
    this.contractorService.getContractors(this.paginations_status).subscribe({
      next: (contractors) => {
        ...
        this.id_contractor_param =
          +this.route.snapshot.queryParamMap.get("id_contractor");
    ...
    this.procedureService.getProcedures().subscribe({
      next: (procedure) => {
        ...
        if (item.name === "RBT") {
          ...
          this.procedureList = procedure.filter(
            (x) => x.name === "H2019" || x.name === "H2012"
          );
          // this.contractorForm.get('payroll')['controls'][index].value.procedure = [];
        }
  ...
  onDeleteContractor(index: number): void {
    ...
    this.contractorService
      .deleteContractor(this.contractors_list[index].id)
      .subscribe(
        (x) => {
  ...
  onSubmit() {
    ...
    try {
      ...
      if (!this.edit_mode) {
        this.contractorService
          .createContractor(contractor as Contractor)
          .subscribe(
            (x) => {
              this.notificationService.successMessagesNotification(
                "Add contractor"
        ...
        this.contractorService
          .updateContractor(contractor as Contractor, this.partial_modify)
          .subscribe(
            (x) => {
  ...
  searchByName() {
    this.spinnerService.show();
    this.contractorService.getContractortByName(this.filterName).subscribe(
      (x) => {
  ...
  adapterPayrollAdd = (data) => ({
    contractorType: null,
    contractorTypeId: Array.isArray(data.contractorType)
      ? data.contractorType[0].id
      : data.contractorType.id,
    procedure: null,
    procedureId: Array.isArray(data.procedure)
      ? data.procedure[0].id
      : data.procedure.id,
    company: null,
    companyId: Array.isArray(data.company)
      ? data.company[0].id
      : data.company.id,
  });
